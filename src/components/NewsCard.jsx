...
function NewsCard({ organization, content, date, type, image, title, liked, toggleLike, setDetailOpen: setParentDetailOpen }) {
  ...
  // ---------------- Portal Overlay ----------------
  const DetailOverlay = () => (
    <div data-no-close className="fixed inset-0 bg-white z-50 flex flex-col mt-16">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="flex items-center justify-between w-full max-w-4xl mx-auto mb-3">
          ...
          <span className={`px-3 py-1 text-xs font-medium rounded-full shadow-sm ${typeColors[type?.toLowerCase()] || "bg-gray-100 text-gray-600"}`}>
            {type}
          </span>
        </div>
  ...
  return (
    <div id={cardId} className="w-full max-w-sm mx-auto mb-6">
      ...
      <div className="bg-white rounded-2xl shadow-md hover:shadow-2xl transition overflow-hidden flex flex-col relative">
        ...
        {image && <img src={image} alt={title} className="w-full h-48 object-cover hover:scale-105 transition-transform duration-200 rounded-t-2xl" />}
        ...
        <div className="p-4 flex flex-col">
          ...
          <div className="flex justify-between items-center text-sm border-t pt-3">
            ...
            <button onClick={() => setDetailOpen(true)} className="relative px-4 py-2 text-sm uppercase tracking-wider text-gray-900 group">
              <span>Read More</span>
              <span className="absolute left-0 bottom-0 w-0 h-0.5 bg-gray-900 transition-all duration-150 group-hover:w-full"></span>
            </button>
          </div>
        </div>
      </div>

      {/* Render detail in portal */}
      {detailOpen && createPortal(<DetailOverlay />, document.body)}
    </div>
  );
}
