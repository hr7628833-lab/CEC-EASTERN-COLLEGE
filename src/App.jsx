...
function App() {
  ...
  const [newsData, setNewsData] = useState(
    Array.from({ length: 15 }, (_, i) => ({
      ...
      date: new Date(`2025-10-${10 + i}`).toISOString(), // initial date
      type: i % 3 === 0 ? "Event" : "Update",
      title: `News Title ${i + 1}`,
      image: `https://picsum.photos/400/300?random=${i + 1}`, // added image
    }))
  );
  ...
  return (
    <Router>
      ...
      <Routes>
        ...
        <Route path="/" element={
          <>
            <div id="header-section" className="relative text-center bg-cover bg-center bg-no-repeat w-full" style={{ backgroundImage: `url(${cec_mainpage})` }}>
            ...
            <main className="flex-grow bg-gradient-to-br from-[#b2b3ff] via-white to-[#eaf1ff] px-6 sm:px-12 py-12">
              ...
              <div className="flex justify-center items-center gap-2 mt-8">
                <button onClick={handlePrevPage} disabled={currentPage === 1} className="px-4 py-2 bg-blue-600 text-white rounded disabled:bg-gray-400">
                  Previous
                </button>
                {Array.from({ length: totalPages }, (_, i) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded ${currentPage === i + 1 ? "bg-blue-800 text-white" : "bg-blue-200 text-blue-700"}`}
                  >
                    {i + 1}
                  </button>
                ))}
                <button onClick={handleNextPage} disabled={currentPage === totalPages} className="px-4 py-2 bg-blue-600 text-white rounded disabled:bg-gray-400">
                  Next
                </button>
              </div>
            </main>
        ...
        {/* Calendar Page */}
        <Route path="/calendar" element={<EventCalendar newsEventsData={newsData} />} />
